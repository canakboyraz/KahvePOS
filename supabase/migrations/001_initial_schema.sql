-- KahvePOS Supabase Database Schema
-- Bu SQL script'i Supabase SQL Editor'da çalıştırın

-- Kullanıcılar tablosu (Supabase Auth ile entegre çalışacak)
...
-- Kasa hareketleri tablosu ( Cash Tracking )
...
-- Müşteriler tablosu (CRM için hazırlık)
...
CREATE POLICY "Tüm authenticated kullanıcılar ürünleri görebilir" ON products
...
-- Fonksiyonlar: Güncelleme zamanlarını otomatik güncelle
...
-- Trigger: Auth user oluşturulduğunda çalışır
CREATE TRIGGER on_auth_user_created
    AFTER INSERT ON auth.users
    FOR EACH ROW EXECUTE FUNCTION public.handle_new_user();

-- Görünüm: Günlük satış özeti
...
-- Görünüm: Ürün performansı (Basitleştirilmiş ve düzeltilmiş)
...
SELECT
    ...
    COALESCE(SUM(
        CASE
            WHEN s.items::text LIKE '%' || p.id::text || '%' THEN
                (s.items->0->>'quantity')::int
            ELSE 0
        END
    ), 0) as total_quantity,
    COALESCE(SUM(
        CASE
            WHEN s.items::text LIKE '%' || p.id::text || '%' THEN
                (s.items->0->>'quantity')::int * p.price
            ELSE 0
        END
    ), 0) as total_revenue
FROM products p
LEFT JOIN sales s ON s.items::text LIKE '%' || p.id::text || '%'
GROUP BY p.id, p.name, p.category, p.price, p.cost, p.is_active
